...
def batch_rename(folder, prefix="file", pattern=r"\d+"):
    ...
    all_sub_folders = check_folders(folder_abs)
    ...
    if all_sub_folders:
        for subfolder in all_sub_folders:
            current_stream = all_sub_folders[subfolder]
            folder_abs = validate_folder_and_files(current_stream, filetype)
            ...
            # 步驟4：生成重命名映射
            rename_mapping = generate_rename_mapping(current_stream, to_rename_files, prefix, used_nums, filetype)
